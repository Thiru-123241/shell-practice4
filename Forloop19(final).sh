...
Y="\e[33m"
C="\e[36m"
PACKAGE=("mysql" "mongodb-org" "nginx")
...
do
 ...
 if [ $? -ne 0 ]
 then
   echo -e "$Y now we are ready to install $i $N" | tee -a $Log_File
   dnf install $i -y &>>$Log_File
  VALIDATE $?  $i
 else
   echo -e "$C $i package is already installed.......nothing to do now $N" | tee -a $Log_File
 fi
 done
